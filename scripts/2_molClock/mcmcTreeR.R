...
setwd("~/Desktop/PhD/0_Thesis/2_chapter/data/molClock_2309/mcmcTree/")
#---- Setting names --------------------------------------------------------------------------------

files = list(fasta = "all_filtered_align-linsi_trim05.fasta",
			 # A newick tree file with no branch length and nodes for calibrations annotated
			 treeToCalib = "all_filtered_align-linsi_trim05_raxml-ng_MCMCTree_MC07_toCalibrate.tre", 
			 # The calibrations file
			 calibrations = "../calibrations_mc08.tsv",
			 # The calibrated file for mcmcTree
			 treeCalib = "all_filtered_align-linsi_trim05_raxml-ng_MCMCTree_MC07_calibrated.tre",
			 timeUnit = 100,
			 minTips = 0)
...
treei = read.beast(files$treeToCalib)
...
for(i in names(nodes)){
    ...
    calibrations$method = cbind(calibrations$method, ss$distribution)
...
output = estimateSkewNormal(phy = tree, 
                      ...
                      maxAge = calibrations$max,
                      monoGroups = monophyleticGroups,
                      writeMCMCtree = TRUE, MCMCtreeName = files$treeCalib)

pdf(gsub("\\.[^\\.]+$", ".pdf", files$treeCalib), width=11.69, height=8.27, paper='special')
...
nodes = list(rads=713, 
			 ...
			 AcanRadA=833, AcanthariaSensuLato=886, AcanthariaSensuStricto=895, AcanSymb=959, RadA=834)
...
for(clade in unique(datam$variable)){
	...
	hpd = hdi(ss$value)
	# cat("  ", clade, "\t", round(quantile(ss$value, 0.5)*factor), "\t(", round(hpd[2]*factor), "-", round(hpd[1]*factor), ")\n", sep="")
	cat("  ", clade, "\t", round(mean(ss$value)*factor), "\t(", round(hpd[2]*factor), "-", round(hpd[1]*factor), ")\n", sep="")
...
# phy = readMCMCtree("all_filtered_align-linsi_trim05_MC7_mcmc.tre")
# priorTree=readMCMCtree("step-prior/all_filtered_align-linsi_trim05_MC7_step-prior.tre")
prior=fread("step-prior/all_filtered_align-linsi_trim05_MC7_step-prior.txt.gz")
