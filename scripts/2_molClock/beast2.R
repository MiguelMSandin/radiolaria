...
files = list(trees = "all_filtered_align-linsi_trim05_beast2_mc07.trees",
			 tree = "all_filtered_align-linsi_trim05_beast2_mc07_mcmc_annot.tre",
			 log = "all_filtered_align-linsi_trim05_beast2_mc07.log",
			 calibrations = "../calibrations_mc07.tsv",
			 plot="all_filtered_align-linsi_trim05_beast2_mc07_prior-posterior.pdf")
...
pdf(files$plot, width=11.69, height=8.27, paper='special')
plot(plotPriPos)
dev.off()
...
file = "all_filtered_align-linsi_trim05_beast2_mc07_summary_nodeHeights.tsv"
# data = fread(file)
...
nodes = list(rads=3, 
			 ...
			 AcanRads=346, RadsBC=347, RadB=348, RadC=447,
			 AcanRadA=465, AcanthariaSensuLato=466, AcanSymb=473, RadA=606)


system(paste("cut -f", paste(nodes, collapse=","), file, "> tmp.tsv"))
datas = fread("tmp.tsv")
system("rm -f tmp.tsv")

colnames(datas) = names(nodes)

pdf("distributions.pdf", width=11.69, height=8.27, paper='special')
...
pdf("distributions_HPD_all.pdf", width=11.69, height=8.27, paper='special')
plot(plotDist)
dev.off()
#
